define(function(require, exports, module) {
	function UploadPic(options) {
		...
		this.selfType = this.mime[options.type];
		this.fileSize = 0;
		this.fileDate = null;
		this.fileType = '';
		this.fileName = '';
	 ...
	 */
	UploadPic.prototype._handelSelectFile = function(file) {
		this.type = file.type
	...
	UploadPic.prototype._drawImage = function(img, callback) {
		...
		this.callback(this.canvas.toDataURL(this.selfType || this.type), this.type);
	...
	return function(e, callback) {
		if (e.target.files) {
			...
			var nUploadPic = new UploadPic({
				callback: callback.bind(this),
				maxSize: 100 * 1024 * 1024,
				type: 'jpg'
			});
			nUploadPic._handelSelectFile(file);
		} else {
			return
		}
	}
